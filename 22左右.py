...
CALIB = {
    "CH1": {"white": 112, "black": 1568},
    "CH2": {"white": 86,  "black": 636},
    "CH3": {"white": 86,  "black": 619},
    "CH4": {"white": 84,  "black": 913},
    "CH5": {"white": 107,  "black": 1955},
}
# ===============================================================
...
# ================= 长直线末端保护 =================
# 作用：连续居中行驶一段时间后，认为小车处在长直线末端，
#      提前把基础速度压低，防止长直线后直角弯冲出去。
LONG_STRAIGHT_PROTECT_ENABLE = True

# pos 小于这个值，认为车基本处在直线中心
LONG_STRAIGHT_CENTER_TH = 0.12
...
# 如果直角还是冲出去：降到 55
# 如果能过但太慢：升到 65
LONG_STRAIGHT_SPEED = 60

# 当 pos 大于这个值，说明已经明显进入弯道，清除长直线保护
LONG_STRAIGHT_RESET_TH = 0.45

long_straight_start_ms = 0
long_straight_protect_active = False
# ===============================================================
...
def calc_line_pos(adc_data):
    ...
    is_crossroad = (signals["CH1"] > 0.6 and signals["CH5"] > 0.6)
...
def motor_control_pid(pos, is_lost):
    ...
    global long_straight_start_ms, long_straight_protect_active

    # 1. 丢线救车逻辑
    if is_lost:
        integral = 0

        # 丢线后清除长直线保护，避免救车阶段还被限速逻辑影响
        long_straight_start_ms = 0
        long_straight_protect_active = False
    ...
    # ================= 长直线末端保护判断 =================
    # 如果长时间 pos 接近 0，说明小车正在长直线高速行驶；
    # 连续居中超过设定时间后，提前限速，避免长直线后直角弯冲出去。
    if LONG_STRAIGHT_PROTECT_ENABLE:
        now = time.ticks_ms()

        if abs(pos) < LONG_STRAIGHT_CENTER_TH:
            if long_straight_start_ms == 0:
                long_straight_start_ms = now

            if time.ticks_diff(now, long_straight_start_ms) >= LONG_STRAIGHT_TIME_MS:
                long_straight_protect_active = True

        elif abs(pos) > LONG_STRAIGHT_RESET_TH:
            # 明显进入弯道后，清除本次长直线保护
            long_straight_start_ms = 0
            long_straight_protect_active = False
    # =====================================================
    ...
    # 长直线末端保护：只限制基础速度，不改变 PID 转向
    if long_straight_protect_active and current_base_speed > LONG_STRAIGHT_SPEED:
        current_base_speed = LONG_STRAIGHT_SPEED
...
def main():
    ...
    print("Long straight protection enabled.")
    time.sleep(2)
